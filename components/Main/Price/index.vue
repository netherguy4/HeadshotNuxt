      ...
      >
        ...
        <div class="price__content">
          ...
          <FormKit
            ...
            :max="[Number(dateFormatted[0]) + 1, ...dateFormatted.slice(1)].join('-')"
            label="Дата:"
            :classes="{
              wrapper: 'price__input-wrapper',
              input: 'price__input',
          ...
          <div class="price__bullets-wrapper">
            ...
            <ul class="price__bullets-list" v-auto-animate>
              <li v-for="label in currentLabels" :key="label" class="price__bullets-label">
                {{ label }}<span> шт</span>
              </li>
            </ul>
          </div>
...
const date = new Date()
const day = new Intl.DateTimeFormat('en-uk', { day: '2-digit', month: '2-digit', year: 'numeric' })
...
const submitHandler = async (data) => {
  ...
  await telegram.send(query).then((res) => {
    if (res.ok) {
      reset('price-form')
      toast.success('Замовлення вiдправлено')
    } else {
      toast.error('Помилка вiдправки замовлення')
    }
  })
}
...
.price {
  ...
  &__submit {
    ...
    &-help {
      ...
      color: #019dce;
      font-weight: 600;
      background: url('./img/ovals.svg') center no-repeat;
      position: relative;
      padding: 50px 60px 35px;
      margin: -50px -60px -35px -60px;
      @media (min-width: $L) {
        background: url('./img/ovals.svg') calc(100% - 15px) center no-repeat;
        text-align: right;
        margin-right: 122px;
      }
      &::before {
